...
int ExecuteByRule::execute(Bedpe & res, Bedpe & truth, char * outputfile, char * rulefile, vector<evaluate_t> & evaluates, Gene & g, int resolution, pseudo_counts_t & pct)
{
  string line;
  vector<rule_t> rtv;
  ...
  for(int i=0;i<rtv.size();i++)
  {
      ...
      evaluate_t et; 
      et.name=rtv[i].name;
      bcomp.compare(outRes, outTruth, g, et, resolution, pct);
