...
int Printer::print(vector<evaluate_t> & evals, char * outputfile,  string gene_file, int base_resolution, string pseudo_counts, string version)
{
    ofstream ofs;
    ofs.open (outputfile, std::ofstream::out);

    ofs<<"#fusionToolEvaluator version "+version<<endl;
    ofs<<"#--gene-annotation-file="+gene_file+"; --base-resolution="+my_db2string(base_resolution)+"; --pseudo-counts="+pseudo_counts<<endl;
